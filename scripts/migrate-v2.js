  ...
  const [rows] = await connection.execute(
    `SELECT COUNT(*) AS count
     ...
     WHERE TABLE_SCHEMA = ?
       AND TABLE_NAME = ?
       AND INDEX_NAME = ?`,
    [process.env.DATABASE_NAME || 'rhulanituckshop', table, indexName]
  );
  return Number(rows[0].count) > 0;
}
...
async function runMigration() {
  ...
  try {
    ...
    // Define the migration statements
    const statements = [
      // 1. Update sales table to add missing transaction types
      `ALTER TABLE sales 
       MODIFY COLUMN transactionType ENUM('sale', 'withdrawal', 'voucher', 'void', 'return') DEFAULT 'sale'`,

      // 4. Create transaction_audit_log table
      ...
      // 5. Create reconciliation_history table
    ...
    const extraColumns = [
      { table: 'till_management', column: 'startedBy', sql: 'startedBy VARCHAR(255)' },
      { table: 'till_management', column: 'closedBy', sql: 'closedBy VARCHAR(255)' },
      { table: 'till_management', column: 'expectedCash', sql: 'expectedCash DECIMAL(10, 2)' },
      { table: 'till_management', column: 'countedCash', sql: 'countedCash DECIMAL(10, 2)' },
      { table: 'till_management', column: 'reconciliationNotes', sql: 'reconciliationNotes TEXT' },
      { table: 'till_management', column: 'reconciliationStatus', sql: "reconciliationStatus ENUM('pending', 'verified', 'discrepancy') DEFAULT 'pending'" },
      { table: 'till_management', column: 'reconciliationApprovedBy', sql: 'reconciliationApprovedBy VARCHAR(255)' },
      { table: 'till_management', column: 'reconciliationApprovedAt', sql: 'reconciliationApprovedAt DATETIME' },
      { table: 'till_management', column: 'difference', sql: 'difference DECIMAL(10, 2)' },
      { table: 'till_management', column: 'varianceReason', sql: 'varianceReason TEXT' },
    ];

    const extraIndexes = [
      { table: 'till_management', name: 'idx_reconciliationStatus', sql: 'reconciliationStatus' },
      { table: 'till_management', name: 'idx_startedBy', sql: 'startedBy' },
      { table: 'till_management', name: 'idx_closedBy', sql: 'closedBy' },
      { table: 'till_management', name: 'idx_reconciliationApprovedAt', sql: 'reconciliationApprovedAt' },
    ];

    console.log(`🚀 Running ${statements.length} migration statements...\n`);

    for (const columnInfo of extraColumns) {
      if (!(await columnExists(connection, columnInfo.table, columnInfo.column))) {
        await connection.execute(`ALTER TABLE ${columnInfo.table} ADD COLUMN ${columnInfo.sql}`);
      }
    }

    for (const indexInfo of extraIndexes) {
      if (!(await indexExists(connection, indexInfo.table, indexInfo.name))) {
        await connection.execute(`ALTER TABLE ${indexInfo.table} ADD INDEX ${indexInfo.name} (${indexInfo.sql})`);
      }
    }
